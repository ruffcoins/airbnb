...
const Sidebar = () => {
	...
	return (
		<div className={`${openSidebar ? "w-56" : "w-[88px]"} border-r transition-width duration-500 ease-in-out`} onMouseEnter={() => (setOpenSidebar(true))} onMouseLeave={() => setOpenSidebar(false)}>
			<div className="flex flex-nowrap items-center justify-start pl-8 h-12 my-3 space-x-4 hover:bg-[#F5F5F5] border-r-[3px] border-r-red-500" style={{ whiteSpace: "nowrap", overflow: "hidden" }}>
				<img className="w-6" src={AllHomes} alt="" />
				{openSidebar && <p className="text-sm text-black font-medium">All Homes</p>}
			</div>

			<div className="flex flex-nowrap items-center justify-start pl-8 my-3 space-x-4 h-12 hover:bg-[#F5F5F5]" style={{ whiteSpace: "nowrap", overflow: "hidden" }}>
